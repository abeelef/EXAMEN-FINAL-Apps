...
class Book(SQLAlchemyBase, JSONModel):
    __tablename__ = "books"
    
    id = Column(Integer, primary_key=True)
    tittle = Column(Unicode(100), unique=True)
    description = Column(UnicodeText)
    author = Column(Unicode(30))
    genre = Column(Unicode(30))
    height = Column(Unicode(30))
    publisher = Column(Unicode(30))
    ...
    def custom(self):
        return {
            "tittle":self.id,
            "description": self.tittle, 
            "author": self.description,
            "genre": self.genre,
            "height": self.height,
