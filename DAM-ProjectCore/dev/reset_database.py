...
from db.models import SQLAlchemyBase, User, GenereEnum, UserToken, Event, EventTypeEnum, Book, Task
...
if __name__ == "__main__":
    ...
    mylogger.info("CREANT LLIBREs")
   
 
    with open('/app/dev/books.csv') as csv_file:
        csv_reader = csv.reader(csv_file,delimiter=",")
        i = 0
        for row in csv_reader:
            if i == 0: #primera fila buida
                i = i+1
            else:               
                book = Book(
                    id = i,
                    tittle=row[0],
                    description="La misma pa todos",
                    author=row[1],
                    genre=row[2],
                    height=row[3],
                    publisher=row[4]
